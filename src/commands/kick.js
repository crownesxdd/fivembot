  ...
  async execute(client, interaction, config, db) {
    ...
    if(interaction.member.permissions.has(PermissionsBitField.Flags.KickMembers)) {
      ...
      if(member.user.id.includes(client.user.id)) {
        return interaction.followUp({ embeds: [
        new EmbedBuilder().setColor(0x2F3136).setAuthor({ name: `${user.tag}`, iconURL: `${user.displayAvatarURL()} ` }).setDescription("<:carpi:1040649840394260510> **|** Bu komutu `Kendime` karşı kullanamazsın.")
     ]});
      }
